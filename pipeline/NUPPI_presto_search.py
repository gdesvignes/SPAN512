...
def get_folding_command(cand, obs, ddplans):
    ...
    if p < 0.002:
        ...
        Mp, Mdm, N = 1, 1, 200
        otheropts = "-npart 24 -nopdsearch -pstep 1 -pdstep 2 -dmstep 1 -nsub 16"
...
def main(filename, workdir):
    ...
    cmd = "rfifind -psrfits -intfrac 0.25 -timesig %.2f -freqsig %.2f -time %.17g -o %s %s > %s_rfifind.out"%\
          (rfifind_timesig, rfifind_freqsig, rfifind_chunk_time, job.basefilename,
    ...
    for ddplan in ddplans:
        ...
        for passnum in range(ddplan.numpasses):
            ...
            for dmstr in ddplan.dmlist[passnum]:
                ...
                cmd = "single_pulse_search.py -f -p -m %f -t %f %s"%\
